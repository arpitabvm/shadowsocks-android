...
class SubscriptionService : Service(), CoroutineScope {
    companion object {
        ...
        val notificationChannel
            @RequiresApi(26) get() = NotificationChannel(
    ...
    override val coroutineContext =
        SupervisorJob() + CoroutineExceptionHandler { _, t -> Timber.w(t) }
        ...
        if (worker == null) {
            ...
            if (!receiverRegistered) {
                ContextCompat.registerReceiver(
                    this, cancelReceiver, IntentFilter(Action.ABORT),
            ...
            worker = launch {
                ...
                val notification =
                    NotificationCompat.Builder(this@SubscriptionService, NOTIFICATION_CHANNEL)
                        .apply {
                            color = ContextCompat.getColor(
                                this@SubscriptionService,
                                R.color.material_primary_500
                            )
                            ...
                            addAction(
                                NotificationCompat.Action.Builder(
                                    ...
                                    PendingIntent.getBroadcast(
                                        this@SubscriptionService,
                                        0,
                                        Intent(Action.ABORT).setPackage(packageName),
                            ...
                            setContentTitle(
                                getString(
                                    R.string.service_subscription_working,
                                    0,
                                    urls.size()
                                )
                            )
                ...
                counter = 0
                val workers =
                    urls.asIterable().map { url -> fetchJsonAsync(url, urls.size(), notification) }
            }
        } else stopSelf(startId)
        return START_NOT_STICKY
    }

    private fun fetchJsonAsync(url: URL, max: Int, notification: NotificationCompat.Builder) =
        async(Dispatchers.IO) {
            ...
            try {
                ...
                launch(Dispatchers.Main) {
                    Toast.makeText(this@SubscriptionService, e.readableMessage, Toast.LENGTH_LONG)
                        .show()
                }
